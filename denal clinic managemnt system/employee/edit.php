...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $stmt = mysqli_prepare($conn, 'UPDATE employee SET first_name = ?, email = ?, password = ? WHERE email = ?');
...
<body>
  <style>
    ...
    .top button{
      ...
      padding: .5rem 1rem;
      border-radius: 2rem;
      box-shadow: 10px 1px 21px 0px rgba(0,0,0,0.39);
    }
  </style>
</head>
<body>
...
<input type="text" id="name" name="name" value="<?php echo isset($user) ? $user['first_name'] : ''; ?>" disabled>
<label for="email">Email:</label>
<input type="email" id="email" name="email" value="<?php echo isset($user) ? $user['email'] : ''; ?>" disabled>
