...
include_once('conthead.php');
$semail=$_SESSION['user_name'];
?>
  <section class="appont" style="margin-top:5rem">
  <?php 
if(isset($_GET['msg'])){ ?>
...
<?php }
  ...
  if (isset($_POST['appoint'])) {
    ...
    if(empty($errors)) {
      $sql = "INSERT INTO appointment (name, email, preferred_date, preferred_time, reason, doctor_id,online)
       VALUES ('$name', '$email', '$date', '$time', '$reason', '$doctor','online')";
      if(mysqli_query($conn, $sql)) {
        $msg="Appointment created successfully";
        header('location: appoint.php?msg=' . urlencode($msg));
      } else {
...
?>
  ...
  <?php
include_once 'db.php';
// Retrieve the patient's first name and email from the session variables
$email = $_SESSION['user_name'];
$qu = "SELECT first_name FROM patients WHERE email='$email'";
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" onsubmit="validateAndSubmit()">
   ...
   <input type="text" id="name" name="name" value="<?= $first_name ?>" required><br><br>
